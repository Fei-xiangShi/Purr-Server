...
class RealtimeRoutesTest {
    ...
    fun `presence and call lifecycle are delivered to the partner`() = testApplication {
        ...
        coroutineScope {
            ...
            val noActiveCall = realtimeClient.get("/calls/active") {
                header(HttpHeaders.Authorization, "Bearer $userBToken")
            }
            assertEquals(HttpStatusCode.OK, noActiveCall.status)
            assertTrue(!noActiveCall.bodyAsText().contains("\"callId\":\"$callId\""))
